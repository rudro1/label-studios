...
export abstract class Player extends Destructable {
  ...
  playFrom(time: number, loop: { start: number; end: number } | null = null) {
    ...
    this.playRange(this.currentTime, loop?.end);
  }
  ...
  pause() {
    if (this.isDestroyed || !this.audio) return;
    // Fixensy: clear loop FIRST and force audio el pause to win race against updateLoop
    this.loop = null;
    this.stopWatch();
    if (!this.playing) {
      // Force-pause underlying HTML element in case updateLoop just toggled playing=false
      try {
        (this as any).audio?.el?.pause?.();
      } catch {}
  ...
  protected playSelection(from?: number, to?: number) {
    if (from !== undefined || to !== undefined) {
      this.loop = to !== undefined ? { start: from ?? this.currentTime, end: to } : null;
  ...
  protected updateLoop(time: number) {
    if (this.isDestroyed || !this.loop || !this.playing) return;
