...
export class Visualizer extends Events<VisualizerEvents> {
  ...
  private seekMouseDown: { x: number; y: number } | null = null;
  ...
  init(audio: WaveformAudio) {
    ...
    if (hasDecodedData) {
      ...
      // Fixensy: hard-cap maxZoom at 1500 so trackpad pinch can't blow up to absurd %
      this.maxZoom = 1500;
    } else {
      // No decoded data
      this.renderers = [];
  ...
  protected createLayers() {
    ...
    this.createLayer({
      ...
      offscreen: true,
      zIndex: 50, // Fixensy: below regions (101) so segments not washed by difference blend
      height: this.waveformLayerHeight,
      compositeOperation: "source-over", // Fixensy: normal alpha blend, not difference
    });
    ...
    // Regions only
    // Fixensy: regions paint BELOW waveform so waveform peaks always visible inside segments (both themes)
  ...
  private handleSeek = (e: MouseEvent) => {
    ...
    if (
      !this.wf.loaded ||
      this.seekLocked ||
      !(e.target instanceof HTMLCanvasElement && this.wrapper.contains(e.target))
    )
      return;
    if (
      this.seekMouseDown &&
      (Math.abs(e.clientX - this.seekMouseDown.x) > 5 || Math.abs(e.clientY - this.seekMouseDown.y) > 5)
    ) {
      return;
    }
    ...
    const allRegions = (this.wf.regions as any).list ?? this.wf.regions.regions ?? [];
    const regionAtClick = (Array.isArray(allRegions) ? allRegions : Array.from(allRegions)).find(
      (r: any) => r && typeof r.start === "number" && clickedTime >= r.start && clickedTime <= r.end,
    );
    const selectedRegions = this.wf.regions.selected;
    const player = (this.wf as any).player;

    // Fixensy: waveform clicks are transport controls. They must not inherit
    // selected-region loop state, otherwise playback gets trapped inside segments.
    // Keep region selection when clicking/drawing on a segment so Valid/Invalid controls stay visible.
    if (!regionAtClick && selectedRegions.length > 0) {
      selectedRegions.forEach((region) => {
        region.selected = false;
        region.invoke?.("update", [region]);
      });
      this.wf.invoke("regionUpdated", [selectedRegions[0]]);
      this.wf.invoke("regionSelectionCleared");
    }
    (this.wf as any).loop = null;
    if (player) player.loop = null;

    this.playhead.setX(playheadX);
    this.wrapper.classList.add("lsf-audio-seek-active");
    if (this.seekFeedbackTimeout) {
      clearTimeout(this.seekFeedbackTimeout);
    }
    this.seekFeedbackTimeout = setTimeout(() => {
      this.wrapper.classList.remove("lsf-audio-seek-active");
      this.seekFeedbackTimeout = null;
    }, 180);

    // Fast path: already playing -> direct currentTime mutation, no restart chain.
    if (player?.playing && player.audio?.el) {
      player.audio.el.currentTime = clickedTime;
      player.time = clickedTime;
      player.timestamp = performance.now();
      this.wf.invoke("seek", [clickedTime]);
      return;
    }

    // Cold start: start exactly at the clicked pointer and bypass selected regions.
    try {
      if (player?.playFrom) {
        player.playFrom(clickedTime, null);
      } else if (player) {
        player.seek?.(clickedTime);
        if (!player.playing) player.play?.(clickedTime);
      } else if (!this.wf.playing) {
        this.wf.currentTime = clickedTime;
        this.wf.play();
      }
    } catch {
  ...
  private handleMouseDown = (e: MouseEvent) => {
    if (e.offsetY > this.height) return;
    if (!this.wf.loaded) return;
    this.seekMouseDown = { x: e.clientX, y: e.clientY };
  ...
  private handleKeyDown = (e: KeyboardEvent) => {
    ...
    if (isPlus || isMinus) {
      e.preventDefault();
      // Increase zoom sensitivity for keyboard: 1.4x for fast zooming
      const factor = isPlus ? 1.4 : 0.7;
      const targetZoom = this.zoom * factor;

      // Zoom towards the center of the view for keyboard zoom
      const ratio = 0.5;
      const timeRatioAtCenter = this.scrollLeft + ratio / this.zoom;
      ...
      requestAnimationFrame(() => {
        ...
        if (this.zoom !== before && this.zoom > 1) {
          let newScrollLeft = timeRatioAtCenter - ratio / this.zoom;
  ...
  private handleScroll = (e: WheelEvent) => {
    ...
    if (this.isZooming(e)) {
      ...
      // Increased sensitivity for faster, more responsive Matrix-Go style zoom
      const sensitivity = isPinch ? 0.25 : 0.18;
