...
export class Regions {
  ...
  constructor(options: RegionsOptions, waveform: Waveform, visualizer: Visualizer) {
    ...
    this.showLabels = (this.waveform as any).params?.showLabels ?? false;
    this.init();
  }
  ...
  private handleClick = (e: MouseEvent) => {
    ...
    if (
      ...
      if (this.layerGroup.isVisible && region) {
        // Fixensy: only forward click event for region; Visualizer.handleSeek owns
        // seek + play to avoid double-handler races (1s delay + segment-lock bug).
  ...
  private cursorInRegion(e: MouseEvent, region: Segment) {
    ...
    const { container, timelinePlacement, height } = this.visualizer;
    const timelineHeight = this.visualizer.timelineComponentHeight;
